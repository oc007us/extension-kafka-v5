...
/**
 * Provides a {@link TopicSubscriber} to subscribe a {@link Consumer} to topic(s).
 *
...
public abstract class TopicSubscriberBuilder<T extends TopicSubscriberBuilder<T>> {
    ...
    /**
     * Set the Kafka {@code topics} to read {@link org.axonframework.messaging.eventhandling.EventMessage}s from.
     * Defaults to {@code Axon.Events}.
     *
     * @param topics the Kafka {@code topics} to read from
    ...
    /**
     * Add a Kafka {@code topic} to read from.
     *
     * @param topic the Kafka {@code topic} to add to the list of topics
     * @return the current Builder instance, for fluent interfacing
     */
    public T addTopic(String topic) {
        assertThat(topic, name -> Objects.nonNull(name) && !name.isEmpty(), "The topic may not be null or empty");
    ...
    /**
     * Set the Kafka {@code pattern} to read from.
     *
     * @param pattern the Kafka {@code pattern} to read from
     * @return the current Builder instance, for fluent interfacing
     */
