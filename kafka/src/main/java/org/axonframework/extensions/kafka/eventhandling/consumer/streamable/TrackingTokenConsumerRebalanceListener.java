 ...
 * will perform a {@link Consumer#seek(TopicPartition, long)} using the partition offsets in the given
 * {@link KafkaTrackingToken}.
 * <p>
 * This implementation ensures that Axon is in charge of event handling progress by always starting from the beginning
 * of the stream for unknown partitions.
 *
 * @param <K> the key of the records the {@link Consumer} polls
 * @param <V> the value type of the records the {@link Consumer} polls
 * @author Steven van Beelen
 * @since 4.0
 * @deprecated functionality moved to {@link ConsumerSeekUtil#seekToCurrentPositions(Consumer, Supplier, List)}
 * when group id was removed from the consumer.
 */
@Deprecated
...
public class TrackingTokenConsumerRebalanceListener<K, V> implements ConsumerRebalanceListener {
    ...
    /**
     * Create a {@link ConsumerRebalanceListener} which uses a {@link KafkaTrackingToken} through the given
     * {@code tokenSupplier} to consciously {@link Consumer#seek(TopicPartition, long)} the offsets of the partitions
     * assigned to the given {@code consumer}.
     *
     ...
     */
    public TrackingTokenConsumerRebalanceListener(Consumer<K, V> consumer,
                                                  Supplier<KafkaTrackingToken> tokenSupplier) {
     ...
     * known records for the new partition are processed.
     */
    @Override
