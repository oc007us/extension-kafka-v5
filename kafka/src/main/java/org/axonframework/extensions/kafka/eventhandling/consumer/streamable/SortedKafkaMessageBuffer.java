...
public class SortedKafkaMessageBuffer<E extends Comparable<?> & KafkaRecordMetaData<?>> implements Buffer<E> {
    ...
    private volatile Runnable onDataAvailable;
    ...
    /**
     * Sets a callback that will be invoked whenever new data is added to this buffer via {@link #put(Comparable)}.
     * This can be used to notify a consumer (e.g. a {@code MessageStream}) that data is available.
     *
     * @param callback the callback to invoke when data becomes available
     */
    public void setOnDataAvailable(Runnable callback) {
    ...
    private void notifyDataAvailable() {
        Runnable callback = this.onDataAvailable;
        if (callback != null) {
            callback.run();
        }
    }
    ...
    /**
     * {@inheritDoc}
     * Will throw the exception once the buffer is empty.
     */
    @Override
