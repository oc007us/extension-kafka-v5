 ...
 * <p>
 * Uses virtual threads by default (Java 21+) for lightweight concurrency.
 *
 * @param <K> the key of the {@link org.apache.kafka.clients.consumer.ConsumerRecords} polled by the
 *            {@link FetchEventsTask}
 * @param <V> the value type of {@link org.apache.kafka.clients.consumer.ConsumerRecords} polled by the
 *            {@link FetchEventsTask}
...
public class AsyncFetcher<K, V, E> implements Fetcher<K, V, E> {
     ...
     * <p>
     * The {@code pollTimeout} is defaulted to a {@link Duration} of {@code 5000} milliseconds and the
     * {@link ExecutorService} to a virtual-thread-per-task executor.
     *
     * @param <K> the key of the {@link org.apache.kafka.clients.consumer.ConsumerRecords} polled by the
     *            {@link FetchEventsTask}
     * @param <V> the value type of {@link org.apache.kafka.clients.consumer.ConsumerRecords} polled by the
     ...
     * <p>
     * The {@code pollTimeout} is defaulted to a {@link Duration} of {@code 5000} milliseconds and the
     * {@link ExecutorService} to a virtual-thread-per-task executor (Java 21+).
     *
     * @param <K> the key of the {@link org.apache.kafka.clients.consumer.ConsumerRecords} polled by the
     *            {@link FetchEventsTask}
     * @param <V> the value type of {@link org.apache.kafka.clients.consumer.ConsumerRecords} polled by the
    ...
    public static final class Builder<K, V, E> {
        ...
        private ExecutorService executorService = Executors.newThreadPerTaskExecutor(
                Thread.ofVirtual().name("async-fetcher-", 0).factory()
        );
         ...
         * milliseconds.
         *
         * @param timeoutMillis the timeoutMillis as a {@code long} when reading messages from the topic
         ...
         * processing of events. Defaults to {@link OffsetCommitType#AUTO}.
         *
         * @param offsetCommitType {@link OffsetCommitType} enum to specify the offset commit type
         * @return the current Builder instance, for fluent interfacing
         */
        public Builder<K, V, E> offsetCommitType(OffsetCommitType offsetCommitType) {
         ...
         * records. Defaults to a virtual-thread-per-task executor.
         * <p>
